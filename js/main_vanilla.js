		...
		// Collapse
			...
			function toggleSlide(el){
				if(el.getAttribute('data-max-height')) {
					...
					//setStylesForSlide(el, 'data-max-true');
					el.style['transition']  = 'max-height 0.5s ease-in-out';
					el.style.overflowY      = 'hidden';
					el.style.maxHeight      = '0';
			...
			function downSlide(el){
				setStylesForSlide(el);
			}
			function upSlide(el){
				setStylesForSlide(el);
			}
			var collapse = document.getElementsByClassName('vcollapse');

			for(var i = 0; i < collapse.length; i++){
				collapse[i].addEventListener('click', function(e){
					var target = e.target;
					if(target.closest('.vcollapse-header')){
						toggle(this.getElementsByClassName('vcollapse-body')[0]);
					}else{
						return;
					}
				});				
